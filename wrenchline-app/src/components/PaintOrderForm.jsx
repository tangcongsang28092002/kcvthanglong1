...
const emptyOrder = { so_thung: '', model: '', ten_thung: '', so_khung: '', so_khung_gac_tam: '', mo_ta: '' }
const fields = [
...
export default function PaintOrderForm({ onCreated }) {
  ...
  async function handleSubmit(event) {
    ...
    const { error: saveError } = await supabase.from('paint_orders').insert(validRows.map(row => ({
      ...
      status: 'pending', assigned_team: 'Tổ sơn', created_by: profile.id, created_at: new Date().toISOString(),
    })))
    setSaving(false)
